...
function Dashboard({ gamesLoader }: { gamesLoader: GamesLoaderReturnType }) {
    ...
    const onUnBetClick = useCallback((game: Game) => {
        Tezos.wallet
            .at(process.env.REACT_APP_TEZBET_CONTRACT!)
    ...
    const onRedeem = useCallback((game: Game) => {
        Tezos.wallet
            .at(process.env.REACT_APP_TEZBET_CONTRACT!)
        ...
        <Fragment>
            ...
            <Container className="dashboard">
                <Row>
                    <Col>
                        ...
                        <Row xs={1} className="g-4">
                            ...
                            {currentFiltered.sort((a, b) => compareGames(a, b))
                                .map((game) => <PlayingCard game={game}
                                    onBetClick={onBetClick}
                                    onUnBetClick={onUnBetClick} />)}
                        </Row>
                    </Col>
                    <Col>
                        <Row className="dashboard-title">
                        ...
                        <Row xs={1} className="g-4">
                            ...
                            {redeemableFiltered.sort((a, b) => compareGames(a, b))
                                .map((game) => <ResultCard game={game} onRedeem={onRedeem} />)}
                        </Row>
                    </Col>
                </Row>
                <Row className="dashboard-title">
                ...
                <Row xs={1} md={2} className="g-4">
                    {archiveFiltered.length <= 0 && <Col className="dashboard-placeholder">No bet in history</Col>}

                    {archiveFiltered.sort((a, b) => compareGames(a, b))
                        .map((game) => <ResultCard game={game} onRedeem={onRedeem} />)}
                        </Row>
            </Container>
        </Fragment>
